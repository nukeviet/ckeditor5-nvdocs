 ...
 * @returns
 */
export function downcastNVDocsAttribute(
	nvdocsUtils: NVDocsUtils, attributeKeys: Array<string>
			...
			dispatcher.on<DowncastAttributeEvent<ModelElement>>( `attribute:${ attributeKey }:nvdocs`, ( evt, data, conversionApi ) => {
				...
				if ( data.attributeKey == 'type' ) {
					if ( data.attributeNewValue == 'fixed' ) {
						...
						const paddingBottom = ( (
							( modelElement.getAttribute( 'ratio' ) as Array<number> )[ 1 ] /
							( modelElement.getAttribute( 'ratio' ) as Array<number> )[ 0 ]
						) * 100 ).toFixed( 2 );
				...
				if (
					...
					viewWriter.setAttribute( `data-docs-${ data.attributeKey }`, data.attributeKey == 'ratio' ? (
						data.attributeNewValue as Array<number>
					).join( ':' ) : data.attributeNewValue, divOuter );
					return;
				}
